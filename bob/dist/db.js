...
export function isGroupChat(chatJid) {
    const row = db
 ...
 */
export function stripBridgePrefix(content) {
 ...
 */
export function isRoutingBanner(content) {
 ...
 */
export function isSubstantiveBotMessage(content) {
    if (isRoutingBanner(content))
        return false;
    const cleaned = stripBridgePrefix(content);
 ...
 */
export function countCrossBotResponses(chatJid, sinceTimestamp) {
    const rows = db
        .prepare(`SELECT content FROM messages WHERE chat_jid = ? AND id LIKE 'xbot_%' AND timestamp > ?`)
        .all(chatJid, sinceTimestamp);
 ...
 */
export function hasCrossBotMention(chatJid, sinceTimestamp, myName) {
    const rows = db
        .prepare(`SELECT content FROM messages WHERE chat_jid = ? AND id LIKE 'xbot_%' AND timestamp > ?`)
        .all(chatJid, sinceTimestamp);
