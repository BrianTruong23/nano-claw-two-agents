...
export function shouldAgentRunForResearchMode(_mode, _waitForBotResponse) {
    return true;
}
/**
 ...
 */
export function formatResearchModeUserNotice(decision, assistantName, waitForBotResponse) {
    const srcLabel = decision.source === 'command' ? 'slash command' : 'auto';
...
function getRoleInstruction(assistantName, isSecondary) {
    if (isSecondary) {
        return [
